...
4. **Verify.** `npm run check` must pass. UI change: preview on a free port (not 5173, it may be the user's), import fictional data (`npm run sample -- out.csv --profile tiny|minimal|single|foreign|international|corsica|regional|regional-outlier|roundtrips|huge|lyon`) at desktop width and 375 px, console clean. After two failed attempts: comment on the issue and stop, no PR.
